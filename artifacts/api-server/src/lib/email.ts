...
export async function sendEmail(payload: EmailPayload): Promise<void> {
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "CreatorTasks <noreply@creatortasks.in>",
        to: payload.to,
  ...
  return `
    <div style="font-family:sans-serif;max-width:560px;margin:0 auto;color:#18181b;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e4e4e7">
      ...
      <div style="padding:16px 32px;border-top:1px solid #e4e4e7;font-size:12px;color:#71717a">
        CreatorTasks · The AI Content Job Board · <a href="https://creatortasks.in" style="color:#7c3aed">creatortasks.in</a>
      </div>
    </div>`;
}
...
export function emailTaskAccepted(to: string, workerName: string, taskTitle: string, taskId: string) {
  return sendEmail({
    ...
    html: wrap(`
      ...
      <a href="https://creatortasks.in/tasks/${taskId}" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">View Task</a>
    `),
  });
}
...
export function emailWorkSubmitted(to: string, taskTitle: string, taskId: string) {
  return sendEmail({
    ...
    html: wrap(`
      ...
      <a href="https://creatortasks.in/tasks/${taskId}" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Review Submission</a>
    `),
  });
}
...
export function emailTaskApproved(to: string, taskTitle: string, amount: number, taskId: string) {
  return sendEmail({
    ...
    html: wrap(`
      ...
      <a href="https://creatortasks.in/dashboard" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">View Wallet</a>
    `),
  });
}
...
export function emailRevisionRequested(to: string, taskTitle: string, note: string | null, taskId: string) {
  return sendEmail({
    ...
    html: wrap(`
      ...
      <a href="https://creatortasks.in/tasks/${taskId}" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Resubmit Work</a>
    `),
  });
}
...
export function emailWalletDeposit(to: string, amount: number) {
  return sendEmail({
    ...
    html: wrap(`
      ...
      <a href="https://creatortasks.in/dashboard" style="display:inline-block;margin-top:16px;padding:10px 20px;background:#7c3aed;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Go to Dashboard</a>
    `),
  });
}
