...
export function AdminPage() {
  ...
  const [activeSection, setActiveSection] = useState<"stats" | "disputes" | "withdrawals" | "tasks" | "users" | "transactions" | "audit">("stats");
        ...
        {([
          ...
          return (
            <button
              ...
              className={`relative rounded-2xl p-3 sm:p-4 flex flex-col items-center justify-center gap-2 transition-colors text-center border ${
                isActive
                  ? "bg-muted border-foreground/30 ring-1 ring-foreground/10"
            ...
            >
              ...
              <span className={`text-[11px] sm:text-xs font-medium leading-tight ${isActive ? "text-foreground font-semibold" : "text-foreground"}`}>{item.label}</span>
              {(item as any).badge ? (
