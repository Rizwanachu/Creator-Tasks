    ...
    <div className="bg-[#111217] border border-[#1F2228] rounded-2xl p-5 hover:border-purple-500/30 hover:bg-white/5 hover:scale-[1.02] transition-all flex flex-col h-full">
      ...
      <div className="mt-auto flex items-center justify-between pt-4 border-t border-white/5">
        <div className="flex flex-col gap-2">
          {task.creatorName && (
            <span className="text-xs text-zinc-500">By {task.creatorName}</span>
          )}
        ...
        <Button 
          ...
          className={task.status === "open" ? "bg-purple-600 hover:bg-purple-500 text-white rounded-xl" : "bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl"}
        >
          <Link href={`/tasks/${task.id}`}>
            {isCreator ? "Manage" : task.status === "open" ? "View Task" : "View Details"}
          </Link>
        </Button>
      </div>
    </div>
